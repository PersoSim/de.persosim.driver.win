...
typedef class VirtualReaderDriver * PVirtualReaderDriver;
typedef class VirtualReaderDevice * PVirtualReaderDevice;

// This is the GUID of the Smart Card Reader device class
...
public:
	...
	funcTrace (char *func) {
		...
		wsprintf(funcName,L"IN -> %s",funcN);
		OutputDebugString(funcName);
	}
	~funcTrace () {
		wchar_t funcName[500];
		wsprintf(funcName,L"OUT -> %s",funcN);
		OutputDebugString(funcName);
	}
};
