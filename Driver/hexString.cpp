...
int HexInt2String(unsigned input, char* hexString)
{

	HexByte2Chars((char)(input >> 24), hexString);
	HexByte2Chars((char)(input >> 16), hexString + 2);
	HexByte2Chars((char)(input >> 8), hexString + 4);
	HexByte2Chars((char)(input), hexString + 6);
	hexString[8] = '\0';
...
{
	int hexLength = (int) strlen(hexString);
	...
	if (hexLength % 2)
	{
		...
		bytes[0] = (char) HexChar2Int(hexString[0]);
	}
	else
	{
		i = 0;
	...
	for (; i < binLength; i++)
	{
		bytes[i] = (char)HexChar2Int(hexString[2 * i + highNibbleOffset]) << 4;
		bytes[i] += (char)HexChar2Int(hexString[2 * i + lowNibbleOffset]);
	}
...
{
	int retVal = 0;
	int hexStringLength = (int) strlen(hexString);
